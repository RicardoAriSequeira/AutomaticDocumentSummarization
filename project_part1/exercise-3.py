...
def main():
    ...
    vectorizer2 = CustomVectorizer(
        input='fromfiles', stopwords=stopwords, encoding='iso-8859-1',bigrams=True, nphrases=True)
    ...
    MAP1 = 0
    MAP2 = 0
    print('File\t\t\tPrecision Alternative\tRecall Alternative\tF1 Alternative\tPrecision Improved\tRecall Improved\tF1 Improved')
    for doc in listdir('textos-fonte'):
        path = 'textos-fonte/' + doc
        stats = compareApproaches(doc, summary(
            vectorizer1, path,True), summary(vectorizer2, path))
        MAP1 += float(stats[1])
        MAP2 += float(stats[4])
